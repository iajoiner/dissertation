...
%\end{enumerate}
%\end{theorem}
\indent Results in this chapter can be generalized to arbitrary green sequences which we will show in an upcoming paper. Some proofs especially the proof of Lemma \ref{lem:C3L2} will be modified to handle complexities resulting from arbitrariness of these green sequences.\\
...
\indent This section is not used in the rest of the chapter. However it does provide new definitions of stability and semistability that are previously underdiscussed. In the future we might make it a special case of some weaker variant of the Bridgeland stability condition \cite{B07} in some upcoming paper. Moreover the ideas in this section are related to $Hom^{\leq 0}$-backward orthogonal sequences.\\
...
\begin{definition}
$M_1,M_2,\cdots, M_k$ is a \textit{backward $Hom^{\leq 0}$ orthogonal sequence} of Schur objects if $(M_i[\geq 0], M_j) = 0$ for all $i>j$ and any $M_i$ is non-zero.
\end{definition}
\begin{definition}
...
\item If $M_j = M_i[1]$ then $j>i$. Moreover if $Y$ is any object of $\catt$ such that $Y[1]\in\catt$ then $l_Y\leq h_{Y[1]}$.
...
\item For any $m>0$ if $M_j = M_i[m]$ then $j>i$. Moreover if $Y$ is any object of $\catt$ such that $Y[m]\in\catt$ then $l_Y\leq h_{Y[m]}$.
\item For any $m>0$ for any $i>j$ it is true that $Hom(M_i[m],M_j) = 0$.
\end{enumerate}
\end{lemma}
\begin{proof}
