...
\end{tikzpicture}
\begin{example}
\indent Here are two stability functions of $mod kQ$ where $Q$ is the quiver $1\to 2$. In the picture on the left $P_2$, $I_1$ and $P_1$ are all stable. $P_2$ and $I_1$ are stable because they are simples. As for $P_1$ it is stable because its submodule $\P_2$ satisfies $\phi(P_2)<\phi(P_1)$.\\
In the picture on the right $P_2$ and $I_1$ are all stable. $P_2$ and $I_1$ are still stable because they are simples. As for $P_1$ it is unstable because its submodule $\P_2$ satisfies $\phi(P_2)\geq \phi(P_1)$.\\
\end{example}
