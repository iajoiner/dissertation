...
\begin{definition}
Let $Q$ be an acyclic quiver and $B$ its exchange matrix. A $c$-vector is \textit{reachable} if it is a column vector in some $C$ such that some $\begin{bmatrix}\tilde{B}\\C\end{bmatrix}$ that can be obtained from $\begin{bmatrix}B\\-I_n\end{bmatrix}$ through mutations.
\end{definition}
\begin{theorem}
...
\end{definition}
\indent We will discuss more about stability conditions in Chapter \ref{C3}.
...
\begin{definition}
Let $\catc$ be an Abelian or triangulated category, let $M,N$ be objects of $\catc$. A morphism $f: M\to N$ is \textit{irreducible} if for any $L\in\catc$
\end{definition}
\begin{example} Here is the Auslander-Reiten(AR) quiver of $mod kQ$ where $Q$ is the quiver $1\to 2$.
$\begin{tikzcd}
&P_1\arrow[rd] &\\
P_2\arrow[ru]& &I_1\\
